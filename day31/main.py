...
window = Tk()
window.title("Flashy")
window.config(padx=50, pady=50, bg=BACKGRAUND_COLOR)
...
canvas.config(bg=BACKGRAUND_COLOR, highlightthickness=0)
canvas.grid(row=0, column= 0, columnspan=2)

cross_image= PhotoImage(file="images/wrong.png")
uknow_button = Button(image=cross_image, highlightthickness=0, command=next_word)
uknow_button.grid(row = 1, column = 0)
